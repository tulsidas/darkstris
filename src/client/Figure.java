...
/**
 * A class representing a Tetris square figure. Each figure consists of four
 * connected squares in one of seven possible constellations. The figures may be
 * rotated in 90 degree steps and have sideways and downwards movability.
 * <p>
 * 
 * Each figure instance can have two states, either attached to a square board
 * or not. When attached, all move and rotation operations are checked so that
 * collisions do not occur with other squares on the board. When not attached,
 * any rotation can be made (and will be kept when attached to a new board).
 * 
...
public class Figure {
    ...
    */
   public boolean attach(BoardSprite board, int newX, int newY) {
   ...
   public void moveTo(FigureInfo newPos) {
      clear();

      yPos = newPos.getY();
      xPos = newPos.getX();
      orientation = newPos.getRotation();
   ...
   // public Collection<Tuple> getSquares() {
   // Collection<Tuple> ret = new ArrayList<Tuple>(4);
   //
   // for (int i = 0; i < shapeX.length; i++) {
   // Tuple t = new Tuple(xPos + getRelativeX(i, orientation), yPos
   // + getRelativeY(i, orientation));
   // }
   //      
   // return ret;
   // }
